...
const youtubeChannelEmailRecord: Record<string, YoutubeChannelInfo> = succeed.reduce((prev, coming) => {
    prev[coming.channelId] = {
        channelName: coming.channelName ?? '',
        channelHandler: '',
        uuid: coming.uuid ?? '',
        emails: []
    }
    return prev
}, {} as Record<string, YoutubeChannelInfo>)
